...
class SynchroCodeProvider {
    ...
    async _handleUploadEvidence(webview, taskId) {
        if (!taskId)
            return;
        try {
            ...
            // Mostrar progreso
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Subiendo evidencia...' }, async () => {
